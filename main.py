...
def poll_new_messages(room_name: str, since: int):
    ...
    # Преобразуем миллисекунды в UTC datetime
    ...
    for msg in room_messages:
        try:
            # Парсим ISO timestamp и преобразуем в UTC aware datetime
            msg_datetime = datetime.fromisoformat(msg['timestamp'].replace('Z', '+00:00'))
            
            # Если timestamp naive, предполагаем UTC
            if msg_datetime.tzinfo is None:
                msg_datetime = msg_datetime.replace(tzinfo=timezone.utc)
            
            # Теперь оба datetime в UTC и aware - сравнение работает правильно
